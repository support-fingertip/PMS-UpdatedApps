...
export class ActivityDashboard extends Component {
    ...
    async render_dashboards (ev) {
        ...
        // Use OR condition: state='today' OR date_deadline equals today
        const today_activity = await self.orm.call('mail.activity',
            'search_read', [], {
                domain: ['|', ["state", "=", 'today'], ['date_deadline', '=', todayDate]]
            });

        const overdue_activity = await self.orm.call('mail.activity',
            'search_read', [], {
                domain: ['|', ["state", "=", 'overdue'], ['date_deadline', '<', todayDate]]
            });
        ...
        self.len_all = planned_activity.length + done_activity.length +
        today_activity.length + overdue_activity.length;
